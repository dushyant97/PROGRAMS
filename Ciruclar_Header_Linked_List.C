//Program for Circular Header Linked List 
...
void insert_sorted_list()
{
	...
	save=start->link;
	ptr=(start->link)->link;	
	if((start->link)->link==(start->link) || input<(ptr->info))
	{
		new1->link=(start->link)->link;
		(start->link)->link=new1;
		return;
...
{
	int value;
	struct node *loc=NULL,*locp=NULL,*save=start,*ptr=(start->link)->link,*new1;
